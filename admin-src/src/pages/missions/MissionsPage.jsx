...
const GAME_TYPE_LABELS = {
  scratch: 'スクラッチくじ', fortune: 'フォーチュンクッキー',
  tap_battle: 'タップバトル', reaction_speed: 'はんのうそくど',
  math_puzzle: 'けいさんパズル', wordle: 'ひらがなワードル',
  memory_card: 'カード神経衰弱', crossword: 'ひらがなクロスワード',
  word_search: 'もじさがし', number_puzzle: '数字スライド',
  sudoku: 'チャデポ数独', sudoku_4x4: '数独(4×4)',
  sudoku_9x9: '数独(9×9ふつう)', sudoku_9x9_hard: '数独(9×9むずかしい)',
  nurie: 'ぬりえパズル',
}
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button
        onClick={() => {
          const d = new Date(value); d.setDate(d.getDate() + 1)
          if (d.toISOString().slice(0, 10) <= jstToday())
            onChange(d.toISOString().slice(0, 10))
        }}
        className="p-1.5 rounded-lg border border-gray-200 hover:bg-gray-50 text-gray-500 text-sm"
      >→</button>
...
export default function MissionsPage() {
  ...
  const { data: gameLog } = useQuery({
    ...
    queryFn: async () => {
      const { data, count, error } = await supabase
        .from('game_plays')
        .select('game_type, user_id, points_earned, recommended_bonus, energy_spent, created_at, profiles!user_id(nickname)', { count: 'exact' })
        .eq('play_date', date)
        .order('created_at', { ascending: false })
  ...
  const { data: boxLog } = useQuery({
    ...
    queryFn: async () => {
      const { data, count, error } = await supabase
        ...
        .select('user_id, points_awarded, created_at, profiles!user_id(nickname)', { count: 'exact' })
        .eq('open_date', date)
        .order('created_at', { ascending: false })
  ...
  const handleDate = (d) => {
    setDate(d)
    setQuestPage(0); setGamePage(0); setBoxPage(0)
  }
    ...
    return (
      <div className="flex items-center justify-between pt-3 border-t border-gray-100">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-gray-500">{page + 1} / {maxPage + 1}</span>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="card text-center">
                  ...
                  <p className="text-3xl font-bold text-gray-900">{totalQuestCompleted.toLocaleString()}건</p>
                </div>
                <div className="card text-center">
                  <p className="text-sm text-gray-500 mb-1">완료한 유저 수</p>
                  <p className="text-3xl font-bold text-brand">
                    {Math.max(...(questStats ?? []).map(r => Number(r.unique_users)), 0).toLocaleString()}명+
                  </p>
                </div>
              </div>
                    ...
                    <div key={q.key} className="card">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-right shrink-0">
                          <p className={`text-2xl font-bold ${cnt > 0 ? 'text-brand' : 'text-gray-300'}`}>{cnt.toLocaleString()}</p>
              ...
              <div className="card">
                <h2 className="font-semibold text-gray-900 mb-3">
                  퀘스트 완료 로그
                  {questLog?.total > 0 && (
                    <span className="ml-2 text-sm text-gray-400 font-normal">총 {questLog.total.toLocaleString()}건</span>
                  )}
                </h2>
                      ...
                      {(questLog?.rows ?? []).map((r, i) => {
                        ...
                        return (
                          <tr key={i} className="hover:bg-gray-50">
                            <td className="py-1.5 text-xs text-gray-700">{nickOf(r.user_id, r.profiles?.nickname)}</td>
                            <td className="py-1.5 text-xs">{q ? `${q.icon} ${q.title}` : r.mission_key}</td>
                            <td className="py-1.5 text-right text-xs text-gray-400">{timeStr(r.created_at)}</td>
                          </tr>
                        )
                      })}
                      {(questLog?.rows ?? []).length === 0 && (
                        <tr><td colSpan={3} className="py-6 text-center text-gray-400 text-xs">기록 없음</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
            ...
            <>
              <div className="grid grid-cols-3 gap-4">
                <div className="card text-center">
                  ...
                  <p className="text-3xl font-bold text-gray-900">{totalGamePlays.toLocaleString()}건</p>
                </div>
                <div className="card text-center">
                ...
                <div className="card text-center">
                  ...
                  <p className="text-3xl font-bold text-orange-500">+{totalGameBonus.toLocaleString()} P</p>
                </div>
              </div>
              ...
              <div className="card p-0 overflow-hidden">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr>
                      <th className="text-left px-4 py-3 text-xs text-gray-500 font-medium">게임</th>
                      <th className="text-right px-4 py-3 text-xs text-gray-500 font-medium">플레이</th>
                      <th className="text-right px-4 py-3 text-xs text-gray-500 font-medium">유저</th>
                      <th className="text-right px-4 py-3 text-xs text-gray-500 font-medium">포인트</th>
                      <th className="text-right px-4 py-3 text-xs text-gray-500 font-medium">보너스</th>
                      <th className="text-right px-4 py-3 text-xs text-gray-500 font-medium">에너지 소비</th>
                    </tr>
                  </thead>
                      ...
                      return (
                        <tr key={g.game_type} className="hover:bg-gray-50">
                          <td className="px-4 py-2.5">
                            <div className="flex items-center gap-2">
                              {tm && <span className={`text-xs px-1.5 py-0.5 rounded font-medium shrink-0 ${tm.cls}`}>{tm.label}</span>}
                              <span className="text-xs text-gray-700">{GAME_TYPE_LABELS[g.game_type] ?? g.game_type}</span>
                            </div>
                          </td>
                          <td className="px-4 py-2.5 text-right font-medium">{Number(g.play_count).toLocaleString()}</td>
                          <td className="px-4 py-2.5 text-right text-gray-500">{Number(g.unique_users).toLocaleString()}</td>
                          <td className="px-4 py-2.5 text-right text-green-600 font-medium">+{Number(g.total_points).toLocaleString()} P</td>
                          <td className="px-4 py-2.5 text-right text-orange-500">{Number(g.total_bonus) > 0 ? `+${Number(g.total_bonus).toLocaleString()}` : '—'}</td>
                          <td className="px-4 py-2.5 text-right text-gray-400">{Number(g.total_energy).toLocaleString()} ⚡</td>
                        </tr>
                      )
                    })}
                    {(gameStats ?? []).length === 0 && (
                      <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-400 text-sm">기록 없음</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
              ...
              <div className="card">
                <h2 className="font-semibold text-gray-900 mb-3">
                  최근 플레이 로그
                  {gameLog?.total > 0 && (
                    <span className="ml-2 text-sm text-gray-400 font-normal">총 {gameLog.total.toLocaleString()}건</span>
                  )}
                </h2>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {(gameLog?.rows ?? []).map((g, i) => (
                        <tr key={i} className="hover:bg-gray-50">
                          <td className="py-1.5 text-xs text-gray-700">{nickOf(g.user_id, g.profiles?.nickname)}</td>
                          <td className="py-1.5 text-xs">{GAME_TYPE_LABELS[g.game_type] ?? g.game_type}</td>
                          <td className="py-1.5 text-right text-xs text-orange-500">-{g.energy_spent}⚡</td>
                          <td className="py-1.5 text-right text-xs text-green-600 font-medium">
                            +{g.points_earned}P
                            {(g.recommended_bonus ?? 0) > 0 && <span className="text-orange-400 ml-1">🔥+{g.recommended_bonus}</span>}
                          </td>
                          <td className="py-1.5 text-right text-xs text-gray-400">{timeStr(g.created_at)}</td>
                        </tr>
                      ))}
                      {(gameLog?.rows ?? []).length === 0 && (
                        <tr><td colSpan={5} className="py-6 text-center text-gray-400 text-xs">기록 없음</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
          ...
          {boxLoading ? (
            ...
            <>
              <div className="grid grid-cols-4 gap-4">
                {[
                  { label: '박스 개봉', value: Number(boxStats?.open_count ?? 0).toLocaleString() + '개', cls: 'text-gray-900' },
                  { label: '개봉 유저', value: Number(boxStats?.unique_users ?? 0).toLocaleString() + '명', cls: 'text-brand' },
                  { label: '총 지급 P', value: Number(boxStats?.total_points ?? 0).toLocaleString() + ' P', cls: 'text-green-600' },
                  { label: '평균 P', value: Number(boxStats?.avg_points ?? 0) + ' P', cls: 'text-gray-700' },
                ].map(({ label, value, cls }) => (
              ...
              <div className="card">
                <h2 className="font-semibold text-gray-900 mb-3">
                  박스 개봉 로그
                  {boxLog?.total > 0 && (
                    <span className="ml-2 text-sm text-gray-400 font-normal">총 {boxLog.total.toLocaleString()}건</span>
                  )}
                </h2>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {(boxLog?.rows ?? []).map((b, i) => {
                        ...
                        return (
                          <tr key={i} className="hover:bg-gray-50">
                            <td className="py-1.5 text-xs text-gray-700">{nickOf(b.user_id, b.profiles?.nickname)}</td>
                            <td className="py-1.5 text-right font-bold text-brand text-sm">+{pts} P</td>
                            <td className={`py-1.5 text-right text-xs font-medium ${grade.cls}`}>{grade.label}</td>
                            <td className="py-1.5 text-right text-xs text-gray-400">{timeStr(b.created_at)}</td>
                          </tr>
                        )
                      })}
                      {(boxLog?.rows ?? []).length === 0 && (
                        <tr><td colSpan={4} className="py-6 text-center text-gray-400 text-xs">기록 없음</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
