...
export default function ReferralPage() {
  ...
  const { data: eventsResult } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        ...
        .select('id, referrer_id, referee_id, status, ad_watch_count, referee_bonus_granted, referee_bonus_amount, referrer_bonus_granted, referrer_bonus_amount, created_at, referrer:profiles!referrer_id(nickname), referee:profiles!referee_id(nickname)', { count: 'exact' })
  ...
  const updateStatus = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries(['referral-events']); qc.invalidateQueries(['referral-summary']) },
  })
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {total > 0 && <span className="ml-auto text-xs text-gray-400">전체 {total.toLocaleString()}건</span>}
          </div>
          ...
          {total > REF_PAGE && (
            <div className="flex items-center justify-between text-sm text-gray-500">
              ...
              <span>{page + 1} / {Math.ceil(total / REF_PAGE)} 페이지</span>
              <button onClick={() => setPage(p => p + 1)} disabled={(page + 1) * REF_PAGE >= total}
            ...
            {(bonusStats ?? []).map((b, i) => (
              <div key={i} className="flex items-center justify-between py-1.5 border-b border-gray-50 last:border-0">
                ...
                <div className="text-right">
                  <div className="text-sm font-bold text-blue-600">+{Number(b.total_energy).toLocaleString()} E</div>
